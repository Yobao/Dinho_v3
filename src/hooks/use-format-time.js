...
const useFormatTime = () => {
	...
	const handleTime = useCallback((matchStart) => {
		...
		/* 		function TimeConstruct(value, timeKey) {
			this.value = value;
			this.timeKey = timeKey;
		}
		TimeConstruct.prototype.setTest = function () {
			//console.log(testArr);
			return testArr.indexOf(this);
		};
		const newTimeArr = [
			new TimeConstruct(diffDays, "day"),
			new TimeConstruct(diffHours, "hour"),
			new TimeConstruct(diffMinutes, "minute"),
			new TimeConstruct(diffSeconds, "second"),
		].filter((timePart) => {
		...
		const timeArray = [
			...
			{ value: diffSeconds, timeKey: "second" },
		];
